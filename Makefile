...
ifneq (,$(GLFW_config))
	...
	@${MAKE} --no-print-directory -C Libraries/GLFW -f Makefile config=$(GLFW_config) CC=$(CC)
endif

GLEW:
ifneq (,$(GLEW_config))
	@echo "==== Building GLEW ($(GLEW_config)) ===="
	@${MAKE} --no-print-directory -C Libraries/GLEW -f Makefile config=$(GLEW_config) CC=$(CC)
endif

yaml-cpp:
ifneq (,$(yaml_cpp_config))
	@echo "==== Building yaml-cpp ($(yaml_cpp_config)) ===="
	@${MAKE} --no-print-directory -C Libraries/YAMLCPP -f Makefile config=$(yaml_cpp_config) CC=$(CC)
endif

curl-lib:
ifneq (,$(curl_lib_config))
	@echo "==== Building curl-lib ($(curl_lib_config)) ===="
	@${MAKE} --no-print-directory -C Libraries/CURL -f Makefile config=$(curl_lib_config) CC=$(CC)
endif

zlib:
ifneq (,$(zlib_config))
	@echo "==== Building zlib ($(zlib_config)) ===="
	@${MAKE} --no-print-directory -C Libraries/ZLIB -f Makefile config=$(zlib_config) CC=$(CC)
endif

Geometria: GLFW GLEW yaml-cpp curl-lib zlib
ifneq (,$(Geometria_config))
	@echo "==== Building Geometria ($(Geometria_config)) ===="
	@${MAKE} --no-print-directory -C Geometria -f Makefile config=$(Geometria_config) CC=$(CC)
endif
